        ...
        <form method="GET" action="{{ route('gallery') }}" class="mb-8 space-y-4">
            <div class="flex flex-wrap items-center gap-2">
                ...
                @foreach($categories as $cat)
                    <a href="{{ route('gallery', array_merge(request()->except('page'), ['category' => $cat])) }}"
                       class="px-4 py-1.5 rounded-full text-sm font-medium transition
                              {{ request('category', 'Semua') === $cat ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200' }}">
                        {{ $cat }}
                    </a>
                @endforeach
            </div>
